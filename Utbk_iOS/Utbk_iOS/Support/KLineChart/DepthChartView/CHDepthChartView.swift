...
/// - bid: 买方深度
/// - ask: 卖方深度
@objc enum CHKDepthChartItemType : NSInteger {
    case bid
    case ask
}
...
/// - left: 左边
/// - right: 右边
@objc enum CHKDepthChartOnDirection : NSInteger {
    case left
    case right
}
 ...
 *  深度数据元素
 */
@objc open class CHKDepthChartItem: NSObject {
    
    @objc var value: CGFloat = 0                              //数值
    @objc var amount: CGFloat = 0                             //数量
    @objc var depthAmount: CGFloat = 0                        //计算得到的深度
    @IBInspectable var type: CHKDepthChartItemType = .bid               //数据类型
...
open class CHDepthChartView: UIView {
    ...
    /// MARK: - 成员变量
    @IBInspectable var bidStrokeColor = UIColor.green
    @IBInspectable var bidFillColor = UIColor.green
    @IBInspectable var bidLineWidth : CGFloat = 1.0
    
    @IBInspectable var askStrokeColor = UIColor.red
    @IBInspectable var askFillColor = UIColor.red
    @IBInspectable var askLineWidth : CGFloat = 1.0
    ...
    @objc func setNoneReferenceStyle() {
        self.yAxis.referenceStyle = .none;
    }
    ...
    /// 内边距
    @IBInspectable open var padding: UIEdgeInsets = UIEdgeInsets.zero
    
    /// 显示y的位置，默认右边
    @IBInspectable var showYAxisLabel = CHYAxisShowPosition.right
    
    /// 是否把y坐标内嵌到图表仲
    @IBInspectable open var isInnerYAxis: Bool = false
    
    /// 买单在右边
    @IBInspectable var bidChartOnDirection = CHKDepthChartOnDirection.right
    
    /// 是否显示X轴标签
    @IBInspectable var showXAxisLabel: Bool = true
    ...
    /// 显示边线上左下有
    @IBInspectable var chartBorderWidth: UIEdgeInsets = UIEdgeInsets.init(top: 0, left: 0, bottom: 0, right: 0)
...
////            self.chartBorderWidth = self.style.borderWidth
...
extension CHDepthChartView {
    ...
    fileprivate func drawChartFrame() {
        ...
        //画低部边线
        if self.chartBorderWidth.bottom > 0 {
            
            borderPath.append(UIBezierPath(rect: CGRect(x: self.bounds.origin.x + self.padding.left, y: self.bounds.size.height + self.bounds.origin.y, width: self.bounds.size.width - self.padding.left, height: self.chartBorderWidth.bottom)))
            
        }
        
        //画顶部边线
        if self.chartBorderWidth.top > 0 {
            
            borderPath.append(UIBezierPath(rect: CGRect(x: self.bounds.origin.x + self.padding.left, y: self.bounds.origin.y, width: self.bounds.size.width - self.padding.left, height: self.chartBorderWidth.top)))
            
        }
        
        
        //画左边线
        if self.chartBorderWidth.left > 0 {
            
            borderPath.append(UIBezierPath(rect: CGRect(x: self.bounds.origin.x + self.padding.left, y: self.bounds.origin.y, width: self.chartBorderWidth.left, height: self.bounds.size.height)))
            
        }
        
        
        //画右边线
        if self.chartBorderWidth.right > 0 {
            
            borderPath.append(UIBezierPath(rect: CGRect(x: self.bounds.origin.x + self.bounds.size.width - self.padding.right, y: self.bounds.origin.y, width: self.chartBorderWidth.left, height: self.bounds.size.height)))
    ...
    func drawTagGraph(point:CGPoint,item:CHKDepthChartItem){
        ...
        if let view = self.delegate?.depthChartTagView?(chart: self, Selected: item){
            ...
            self.selectedTagGraphslayer!.frame = frame
            if item.type == .bid{
                self.selectedTagGraphslayer!.backgroundColor = self.bidFillColor.cgColor
            }else{
                self.selectedTagGraphslayer!.backgroundColor = self.askFillColor.cgColor
            }
    ...
    func drawChart() -> [(CGRect, String)]{
        ...
        if self.bidChartOnDirection == .right{
            ...
            if let askChartLayer = self.drawDepthChart(items: self.askItems, startIndex: startIndex, strokeColor: self.askStrokeColor, fillColor: self.askFillColor, lineWidth: self.askLineWidth) {
            ...
            if let bidChartLayer = self.drawDepthChart(items: self.bidItems, startIndex: startIndex, strokeColor: self.bidStrokeColor, fillColor: self.bidFillColor, lineWidth: self.bidLineWidth) {
