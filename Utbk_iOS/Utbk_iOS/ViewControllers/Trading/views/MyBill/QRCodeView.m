...
- (void)savedPhotoImage:(UIImage*)image didFinishSavingWithError: (NSError *)error contextInfo: (void *)contextInfo {
    if (error) {
        [self makeToast:LocalizationKey(@"savePhotoFailure") duration:1.5 position:ToastPosition];
    }
    else {
//        NSLog(@"保存图片成功");
        [self makeToast:LocalizationKey(@"savePhotoSuccess") duration:1.5 position:ToastPosition];
    }
}
