...
-(void)addCollectionWithsymbol:(NSString*)symbol{
    [MarketNetManager addMyCollectionWithsymbol:symbol CompleteHandle:^(id resPonseObj, int code) {
        if (code) {
            if ([resPonseObj[@"code"] integerValue] == 0) {
                 [self.moreBtn setImage:[UIImage imageNamed:@"icon_colletion"] forState:UIControlStateNormal];
                _isCollect=YES;
            }else{
                [self.view makeToast:resPonseObj[MESSAGE] duration:1.5 position:ToastPosition];
            }
...
- (IBAction)leftMenu:(UIButton *)sender {
    if (!self.menu) {
        ...
        self.menu.view.frame = CGRectMake(- CGRectGetWidth(self.view.frame), 0,  SCREEN_WIDTH, SCREEN_HEIGHT);
...
//                cell.amountLabel.textColor=RedColor;
            ...
            if (bidplatemodel.amount>=0 && allDepthsellAmount > 0) {
                cell.backwidth.constant = (CGFloat)bidplatemodel.totalAmount/allDepthsellAmount*cell.contentView.width;
            }else{
...
#pragma mark 滑块百分比点击事件
- (IBAction)rateClickAction:(UIButton *)sender {
    if (sender == self.selecetBtn) return;
    self.selecetBtn.selected = NO;
    self.selecetBtn.backgroundColor = [UIColor whiteColor];
    sender.selected = YES;
...
//                [self.view makeToast:LocalizationKey(@"validateYourID") duration:1.5 position:ToastPosition];
//
//                return;
//            }
            if (!_IsSell) { /*买入*/
                ...
                if (self.priceType == PriceType_Fixed) {
                    ...
                    if ( [NSString stringIsNull:self.PriceTF.text]) {
                        [self.view makeToast:LocalizationKey(@"enterprice") duration:1.5 position:ToastPosition];
                        return;
                    }
                    if ([self.PriceTF.text doubleValue]<=0) {
                        [self.view makeToast:LocalizationKey(@"commissionPricezero") duration:1.5 position:ToastPosition];
                        return;
                    }
                    if ( [NSString stringIsNull:self.AmountTF.text]) {
                        [self.view makeToast:LocalizationKey(@"pleasecommissionamount") duration:1.5 position:ToastPosition];
                        return;
                    }
                    if ([self.AmountTF.text doubleValue]<=0) {
                        [self.view makeToast:LocalizationKey(@"commissionamountzero") duration:1.5 position:ToastPosition];
                        return;
                    }
                    __weak TradeViewController*weakSelf=self;
                    EasyShowAlertView *showView =[EasyShowAlertView showActionSheetWithTitle:LocalizationKey(@"commissionbuy") left1message:[NSString stringWithFormat:@"%@%@",[ToolUtil stringFromNumber:[self.PriceTF.text doubleValue] withlimit:_baseCoinScale],_baseCoinName] right1message:[NSString stringWithFormat:@"%@%@",[ToolUtil stringFromNumber:[self.AmountTF.text doubleValue] withlimit:_coinScale],_ObjectCoinName] left2message:[NSString stringWithFormat:@"%@%@",[ToolUtil stringFromNumber:[self.PriceTF.text doubleValue]*[self.AmountTF.text doubleValue] withlimit:_baseCoinScale],_baseCoinName] right2message:LocalizationKey(@"buyDirection")];
                    ...
                    [EasyShowOptions sharedEasyShowOptions].alertMessageColor = NavColor;
                    [showView show];
                }else if (self.priceType == PriceType_Market){
                    //市价买入
                    if ( [NSString stringIsNull:self.AmountTF.text]) {
                        [self.view makeToast:LocalizationKey(@"commissionmoney") duration:1.5 position:ToastPosition];
                        return;
                    }
                    if ([self.AmountTF.text doubleValue]<=0) {
                        [self.view makeToast:LocalizationKey(@"commissionmoneyzero") duration:1.5 position:ToastPosition];
                        return;
                    }
                    ...
                    if ( [NSString stringIsNull:self.triggerTF.text]) {
                        [self.view makeToast:LocalizationKey(@"pleaseenterTrigger") duration:1.5 position:ToastPosition];
                        return;
                    }
                    if ([self.triggerTF.text doubleValue]<=0) {
                        [self.view makeToast:LocalizationKey(@"TriggerPricezero") duration:1.5 position:ToastPosition];
                        return;
                    }
                    if ( [NSString stringIsNull:self.PriceTF.text]) {
                        [self.view makeToast:LocalizationKey(@"enterprice") duration:1.5 position:ToastPosition];
             ...
             /*卖出*/
                if (self.priceType == PriceType_Fixed) {
                    ...
                    //限价卖出
                    if ( [NSString stringIsNull:self.PriceTF.text]) {
                        [self.view makeToast:LocalizationKey(@"pleaseenterPrice") duration:1.5 position:ToastPosition];
                        return;
                    }
                    if ([self.PriceTF.text doubleValue]<=0) {
                        [self.view makeToast:LocalizationKey(@"commissionPricezero") duration:1.5 position:ToastPosition];
        ...
        case 106: //查看全部当前委托
        {
            ...
            break;
            case 200://数量减
            if ([self.AmountTF.text doubleValue] >= 0.01) {
...
-(CGFloat)getviewheightspce{
    ...
    if (self.isHistory) {
        ...
        return 40 + (self.contentArr.count > 0 ? 115 * self.contentArr.count : 133);
