...
- (void)setupBind{
    ...
    if (![YLUserInfo shareUserInfo].avatar.length) {
        self.headerBtn.layer.borderWidth = 0.f;
    }
    ...
    [[XBRequest sharedInstance]postDataWithUrl:getMemberStatusAPI Parameter:@{@"memberId":_BTS([YLUserInfo shareUserInfo].ID)} ResponseObject:^(NSDictionary *responseResult) {
        if (NetSuccess) {
            ...
            NSString *title = (info.activeStatus == 1 || info.activeStatus == 2) ?  (info.activeStatus == 2 ? LocalizationKey(@"已激活") : LocalizationKey(@"网体激活")) : LocalizationKey(@"未激活");
