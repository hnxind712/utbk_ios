...
//        }else{
//            [self.view makeToast:responseObject[MESSAGE] duration:1.5 position:ToastPosition];
//        }
...
- (IBAction)ApplyNowaction:(id)sender {
    if ([NSString stringIsNull:self.phoneTF.text]) {
        [self.view makeToast:LocalizationKey(@"inputMobile") duration:1.5 position:ToastPosition];
        return;
    }
    
    if ([NSString stringIsNull:self.wechatTF.text]) {
        [self.view makeToast:LocalizationKey(@"Pleaseenterthewechat") duration:1.5 position:ToastPosition];
        return;
    }
  
    if ([NSString stringIsNull:self.QQTF.text]) {
        [self.view makeToast:LocalizationKey(@"PleaseentertheQQ") duration:1.5 position:ToastPosition];
        return;
    }
    
    if ([NSString stringIsNull:self.presonProveurl]) {
        [self.view makeToast:LocalizationKey(@"uploadpersonaldigital") duration:1.5 position:ToastPosition];
        return;
    }
    if ([NSString stringIsNull:self.presonProveurl]) {
        [self.view makeToast:LocalizationKey(@"uploadpersonaltransaction") duration:1.5 position:ToastPosition];
        return;
    }
    ...
    [MineNetManager approvecertifiedbusinessParam:jsondic CompleteHandle:^(id resPonseObj, int code) {
        ...
        if (code) {
            if ([resPonseObj[@"code"] integerValue]==0) {
               [self.view makeToast:LocalizationKey(@"Submitsuccessfully") duration:1.5 position:ToastPosition];
