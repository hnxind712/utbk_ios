...
-(void)ImageUI{
    ...
    for (int i = 0; i < 3; i ++) {
        UIImageView *imageview = [[UIImageView alloc]initWithFrame:CGRectMake( SCREEN_WIDTH * (0.1  + i), 0, SCREEN_WIDTH * 0.8,  SCREEN_WIDTH * 0.8)];
...
- (IBAction)Applyaction:(id)sender {
    ...
    if (!self.agreebutton.selected) {
        [self.view makeToast:LocalizationKey(@"PleaseUserprotocol") duration:1.5 position:ToastPosition];
        return;
    }
