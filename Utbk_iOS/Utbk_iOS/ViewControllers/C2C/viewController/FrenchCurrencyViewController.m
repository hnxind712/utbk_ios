...
- (void)setChildViewControllers{
    ...
    indus.view.frame = CGRectMake(0, 0, SCREEN_WIDTH, self.mainScrollView.frame.size.height);
...
- (void)ybPopupMenu:(YBPopupMenu *)ybPopupMenu didSelectedAtIndex:(NSInteger)index
{
    ...
    if (index == 4) {
        ...
        if (!self.fundsVerified) {
            [self.view makeToast:LocalizationKey(@"bindingPwd") duration:1.5 position:ToastPosition];
            return;
        }
...
//        [[AppDelegate sharedAppDelegate] pushViewController:turn];
    ...
    if (self.memberLevel != 2) {
        [self.view makeToast:LocalizationKey(@"Uncertifiedbusinesses") duration:1.5 position:ToastPosition];
        return;
    }
    if (index == 0) {
        //购买
...
- (UIScrollView *)mainScrollView{
    if (!_mainScrollView) {
        ...
        _mainScrollView.frame = CGRectMake(0, CGRectGetMaxY(self.headerView.frame), SCREEN_WIDTH, SCREEN_HEIGHT - NavBarHeight - TabBarHeight - self.headerView.frame.size.height);
